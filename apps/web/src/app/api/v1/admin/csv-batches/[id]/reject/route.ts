...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.update(csvImportBatches).set({ status: "rejected", rejectionReason: reason, reviewedBy: session.user.id, reviewedAt: new Date() }).where(eq(csvImportBatches.id, id));
